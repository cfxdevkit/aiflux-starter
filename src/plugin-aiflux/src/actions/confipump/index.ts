      ...
      switch (contentObject.action) {
        ...
        case 'BUY_TOKEN':
          ...
          data = encodeFunctionData({
            abi: MEMEABI,
            functionName: 'buy',
            args: [contentObject.params.tokenAddress as `0x${string}`, account.address, 0n, false],
          });
          break;
        ...
        case 'SELL_TOKEN':
          ...
          const tokenAddress = getAddress(contentObject.params.tokenAddress as `0x${string}`);
          elizaLogger.debug('selling: ', tokenAddress, account.address, contentObject.params.value);
