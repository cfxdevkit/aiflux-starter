...
export async function executeSwap(
    ...
    try {
        ...
        const path =
            params.fromToken === "CFX" || params.toToken === "CFX"
        ...
        elizaLogger.debug("Swap path determined", {
            ...
            pathType:
                params.fromToken === "CFX" || params.toToken === "CFX" ? "direct" : "via_wcfx",
        });
        ...
        handler: async (
            ...
            try {
                ...
                try {
                    ...
                    const fromToken =
                        actionDetails.object.fromToken.toUpperCase() === "CFX"
                            ? config.tokenListManager.getTokenBySymbol("WCFX")
                            : config.tokenListManager.getTokenBySymbol(
                                  actionDetails.object.fromToken
                              );
                    const toToken =
                        actionDetails.object.toToken.toUpperCase() === "CFX"
                            ? config.tokenListManager.getTokenBySymbol("WCFX")
                            : config.tokenListManager.getTokenBySymbol(
                                  actionDetails.object.toToken
                              );
                    ...
                    try {
                        ...
                        callback(
                            { text: `❌ Swap failed: ${error.message}`, action: "SWAP_ESPACE" },
                            []
                        );
                        return false;
                    }
