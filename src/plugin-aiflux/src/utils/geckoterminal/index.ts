...
export class GeckoTerminal {
   ...
   */
  public async getTopPools(page = 1, dex = this.DEFAULT_DEX): Promise<PoolsResponse> {
  ...
  public async getTokenInfo(tokenAddress: string): Promise<TokenInfo> {
    return this.fetchData<TokenInfo>(`/networks/${this.NETWORK}/tokens/${tokenAddress}`);
  }
   ...
   * @param page Page number (default: 1)
   */
  public async getTokenPools(tokenAddress: string, page = 1): Promise<PoolsResponse> {
    return this.fetchData<PoolsResponse>(
  ...
  public formatPool(pool: any): FormattedPool {
    ...
    const formattedPool = {
      ...
      baseTokenAddress: pool.relationships.base_token.data.id.split('_').pop() || '',
      quoteTokenAddress: pool.relationships.quote_token.data.id.split('_').pop() || '',
