...
export class CoreContractDetector {
    ...
    async detectContract(address: string): Promise<ContractCheckResult> {
        try {
            ...
            if (checksummedAddress === "cfx:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa0sfbnjm2") {
    ...
    private async detectContractType(
        ...
        try {
            ...
            console.debug("Contract doesn't implement ERC20", error);
            // Not an ERC20 token
        }
                ...
                for (const { name, supported } of interfaceChecks) {
                    if (supported) {
                        ...
                        if (!isERC20) {
                            ...
                            else if (name === "ERC1155") result.type = "CRC1155";
                            else if (name === "ERC4626") result.type = "CRC4626";
                        }
