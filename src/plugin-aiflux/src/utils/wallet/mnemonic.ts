...
function isValidMnemonic(mnemonic: string): boolean {
    ...
    elizaLogger.info(`Mnemonic validation result: ${isValid}`);
    return isValid;
}
...
export function deriveESpaceKey(mnemonic: string, index: number = 0): `0x${string}` {
    elizaLogger.info(`Deriving eSpace key for index: ${index}`);
...
export function deriveCoreKey(mnemonic: string, index: number = 0): `0x${string}` {
    elizaLogger.info(`Deriving Core key for index: ${index}`);
    if (!isValidMnemonic(mnemonic)) {
    ...
    const derivedKey = hdKey.derive(derivationPath);
    const privateKey = `0x${Buffer.from(derivedKey.privateKey!).toString("hex")}`;
    elizaLogger.info("Core private key derived successfully");
    return privateKey as `0x${string}`;
}

export function generateMnemonic(): string {
    elizaLogger.info("Generating new 12-word mnemonic");
    const mnemonic = gm(wordlist, 128);
    elizaLogger.info("Mnemonic generated successfully");
    return mnemonic;
}
