...
export class TokenListManager {
    ...
    public async loadFromGeckoTerminal(): Promise<void> {
        ...
        try {
            ...
            for (let i = 0; i < uniqueTokenAddresses.length; i += BATCH_SIZE) {
                ...
                try {
                    const tokenInfos = await this.geckoTerminal.getMultiTokenInfo(batch);
                    ...
                    if ((error as any).status === 429) {
                        ...
                        elizaLogger.error(`Error fetching token info batch:`, error);
                        elizaLogger.error("Failed addresses:", batch);
                    }
                }
            }
            ...
            elizaLogger.error("Error loading tokens from GeckoTerminal:", error);
        }
    }
    ...
    getTopGainers(limit: number = 5): string {
        const sortedPools = [...this.pools].sort(
            (a, b) => parseFloat(b.priceChange24h) - parseFloat(a.priceChange24h)
        );
        return (
            `Top ${limit} Price Gainers (24h):\n\n` +
            sortedPools
                .slice(0, limit)
                .map(
                    (pool) => `${pool.name}: ${pool.priceChange24h}% | Current Price: ${pool.price}`
                )
                .join("\n")
        );
    }
    ...
    getTopLosers(limit: number = 5): string {
        const sortedPools = [...this.pools].sort(
            (a, b) => parseFloat(a.priceChange24h) - parseFloat(b.priceChange24h)
        );
        return (
            `Top ${limit} Price Losers (24h):\n\n` +
            sortedPools
                .slice(0, limit)
                .map(
                    (pool) => `${pool.name}: ${pool.priceChange24h}% | Current Price: ${pool.price}`
    ...
    getPoolsByAge(newest: boolean = true, limit: number = 5): string {
        ...
        return (
            `${newest ? "Newest" : "Oldest"} ${limit} Pools:\n\n` +
            sortedPools
                .slice(0, limit)
                .map(
                    (pool) =>
                        `${pool.name}: Created ${new Date(pool.createdAt).toLocaleDateString()} | Reserve: $${Number(pool.reserveUSD).toLocaleString()}`
                )
                .join("\n")
        );
    }
    ...
    getTopTrades(limit: number = 5): string {
        const sortedPools = [...this.pools].sort((a, b) => b.trades24h.total - a.trades24h.total);
        return (
            `Top ${limit} Pools by 24h Trades:\n\n` +
            sortedPools
                .slice(0, limit)
                .map(
                    (pool) =>
                        `${pool.name}: ${pool.trades24h.total} trades (${pool.trades24h.buys} buys, ${pool.trades24h.sells} sells)`
                )
                .join("\n")
        );
    }
    ...
    getMostBuyPressure(limit: number = 5): string {
        const sortedPools = [...this.pools].sort(
            (a, b) => b.trades24h.buys / b.trades24h.total - a.trades24h.buys / a.trades24h.total
        );
        return (
            `Top ${limit} Pools by Buy Pressure:\n\n` +
            sortedPools
                .slice(0, limit)
                .map((pool) => {
                    const buyPercentage = (
                        (pool.trades24h.buys / pool.trades24h.total) *
                        100
                    ).toFixed(1);
    ...
    getMostSellPressure(limit: number = 5): string {
        const sortedPools = [...this.pools].sort(
            (a, b) => b.trades24h.sells / b.trades24h.total - a.trades24h.sells / a.trades24h.total
        );
        return (
            `Top ${limit} Pools by Sell Pressure:\n\n` +
            sortedPools
                .slice(0, limit)
                .map((pool) => {
                    const sellPercentage = (
                        (pool.trades24h.sells / pool.trades24h.total) *
                        100
                    ).toFixed(1);
